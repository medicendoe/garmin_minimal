...
function testBatteryColorAtThreshold(logger as Test.Logger) as Boolean {
    ...
    Test.assertEqualMessage(0xFF0000, batteryColorFor(15.0f, 0xFFFFFF, 0xFF0000), "15% battery should use alert color");
    return true;
}
...
function testBatteryColorAboveThreshold(logger as Test.Logger) as Boolean {
    ...
    Test.assertEqualMessage(0xFFFFFF, batteryColorFor(16.0f, 0xFFFFFF, 0xFF0000), "16% battery should use normal color");
    return true;
}
...
function testSecondsZeroPad(logger as Test.Logger) as Boolean {
    ...
    Test.assertEqualMessage("05", (5).format("%02d"), "single-digit seconds must be zero-padded");
    return true;
}
